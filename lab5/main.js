...
function makeDrawing(canvas, choice1, x1, y1, choice2, x2, y2, showOrigin) {
    ...
    function safeDraw(funcName, targetCanvas, x, y, showOrigin) {
        if (typeof window[funcName] === "function") {
            try {
                ...
                console.warn(`${funcName} standard call failed. Trying alternative parameter orders...`);
    ...
    // Run animation (only if defined)
    if (typeof switcheroo === "function") {
        switcheroo(item1, x1, y1, item2, x2, y2);
    } else {
        console.warn("switcheroo() function not found.");
    }
}